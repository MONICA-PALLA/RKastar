...
if __name__ == "__main__":
	...
	for RM in order:
		x2.append(x[int(RM)])
		y2.append(y[int(RM)])
	x2.append(x[int(order[0])])
	y2.append(y[int(order[0])])

	
	graph = Graph(amount_vertices=30)

	i = 0
	file = io.open('city_data.txt', 'r')
	...
	pso = PSO(graph, iterations=100, size_population=30, beta=1, alfa=0.9)
...
####	pso.showsParticles() # shows the particles
	...
	plt.plot(x2,y2, marker='x', linestyle = '-', color = 'r', label = 'ACO')
	for RM in range(len(order)):
		plt.text(x2[RM],y2[RM],int(order[RM]+1))
	...
	G=networkx.path_graph(graph)
	print(networkx.astar_path(G,(0,0),(2,2),dist))
