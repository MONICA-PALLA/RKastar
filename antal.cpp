...
//		printf("%d %d\n", ant, ants[ant].curCity);
		...
		ants[ant].tabu[to] = 1;
		to++;
		//printf("%d\n", ants[ant].tabu[ants[ant].curCity]);
	}
	/*for(RK=0; RK < MAX_ANTS; RK++)
	{
		printf("%d\n", ants[RK].tabu[ants[RK].curCity]);
	} */
}
...
void restartAnts()
{
	...
	for(RK = 0; RK<MAX_ANTS; RK++)
	{
		if(ants[RK].tourLength < best)
		{
		
			best = ants[RK].tourLength;
			bestIndex = RK;
		}
		
		ants[RK].nextCity = -1;
		ants[RK].tourLength = 0.0;
...
int main()
{
	...
	int uuu = sizeof(phero)/sizeof(phero[0]);
